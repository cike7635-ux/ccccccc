...
export default function DeleteThemeButton({ themeId, themeTitle, onDelete }: DeleteThemeButtonProps) {
  ...
  useEffect(() => {
    if (showDialog) {
      ...
      // 隐藏所有可能的导航栏和底部栏
      const navElements = document.querySelectorAll('nav, footer, [class*="nav"], [class*="bottom"]');
      ...
      setTimeout(() => {
        if (dialogRef.current) {
          dialogRef.current.style.transform = 'translateZ(0)';
        }
      }, 10);
    } else {
      // 恢复背景滚动
      ...
      const navElements = document.querySelectorAll('nav, footer, [class*="nav"], [class*="bottom"]');
      navElements.forEach(el => {
        if (el instanceof HTMLElement && el.dataset.originalVisibility) {
